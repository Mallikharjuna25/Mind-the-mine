...
from app.models.module2_models import InspectionTemplate, InspectionAudit, FieldVerification
...
def get_templates(db: Session = Depends(get_sync_db)):
...
def get_inspection_by_id(audit_id: str, db: Session = Depends(get_sync_db), current_user: dict = Depends(get_current_user)):
